...
d0 = (
    ...
    .withColumn("subproduto", F.lit("debito contactless"))
    .withColumn("firula", F.lit("14"))
)

d1 = (
    (
        df.withColumn("now", F.current_date())
        .where(F.datediff(F.current_date(), "data_hora") <= F.lit(5))
        .groupBy("optype")
        .agg(
            F.count("valor").alias("count"),
            F.sum("valor").alias("sum"),
    ...
    .withColumn("subproduto", F.lit("debito sem aut"))
    .withColumn("firula", F.lit("31"))
)
d2 = (
    (
        df.withColumn("now", F.current_date())
        .where(F.datediff(F.current_date(), "data_hora") <= F.lit(3))
        .groupBy("optype")
        .agg(
            F.count("valor").alias("count"),
            F.sum("valor").alias("sum"),
    ...
    .withColumn("subproduto", F.lit("debito com aut"))
    .withColumn("firula", F.lit("1"))
)
d3 = (
    (
        df.withColumn("now", F.current_date())
        .where(
            (F.datediff(F.current_date(), "data_hora") <= F.lit(3))
    ...
    .withColumn("subproduto", F.lit("debito com aut"))
    .withColumn("firula", F.lit("2"))
)
d4 = (
    (
        df.withColumn("now", F.current_date())
        .where(
            (F.datediff(F.current_date(), "data_hora") <= F.lit(5))
            & (F.col("dia_semana").isin([1, 7]))
        )
        .groupBy("optype")
        .agg(
...
dx = dx.withColumn(
    "_pivot",
    F.concat_ws("-", "subproduto", "janela", "periodo", "firula", "teste"),
)
dx.show(truncate=False)
